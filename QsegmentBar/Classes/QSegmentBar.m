...
#import "UIView+QSegmentBar.h"


#define kMinMargin 30

@interface QSegmentBar ()
{
    // 记录最后一次点击的按钮
    UIButton *_lastBtn;
}
...
@property (nonatomic, strong) QSegmentBarConfig *config;

@end

@implementation QSegmentBar



#pragma mark - 接口
+ (instancetype)segmentBarWithFrame: (CGRect)frame {
    QSegmentBar *segmentBar = [[QSegmentBar alloc] initWithFrame:frame];
...
- (QSegmentBarConfig *)config {
    if (!_config) {
        _config = [QSegmentBarConfig defaultConfig];
    }
    return _config;
}
